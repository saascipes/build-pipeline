...
export const actions: ActionTree<CoreState, RootState> = {  
  ...
  save({commit, state, dispatch}, model?: Quote) : Promise<Model> {
  ...
  delete({commit, state}, model?: Quote): Promise<void> {
    return coreActions.delete({commit, state}, model);
  },

  select({commit, state, dispatch}, model: Quote): Promise<Model|undefined> {
