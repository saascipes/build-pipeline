...
export const actions: ActionTree<CoreState, RootState> = {  
  ...
  save({commit, state, dispatch}, model?: StockSubscription) : Promise<Model> {
  ...
  delete({commit, state}, model?: StockSubscription): Promise<void> {
    return coreActions.delete({commit, state}, model);
  },

  select({commit, state, dispatch}, model: StockSubscription): Promise<Model|undefined> {
