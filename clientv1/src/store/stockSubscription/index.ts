...
export const state: CoreState = {
  ...
  _url: () => `api/v0/${ModelBaseUrlType.stockSubscription}`,
  _storeName: StoreType.StockSubscriptionStore.toString(),
  _promiseStore: new PromiseStore()
};

state.storeUtils = new StoreUtils(state);

export const stockSubscriptionStore: Module<CoreState, RootState> = {
    namespaced: true,
    state,
    actions,
    mutations
